...
const outputBasename = process.env.FE_STORY_11_5_OUTPUT_BASENAME ?? '11-5-fe-actual-runtime-flow';
...
const port = Number(process.env.FE_STORY_11_5_PORT ?? '4277');
...
const member = {
  memberUuid: 'member-story-11-5-fe',
...
const installMockRoutes = async (page) => {
  let csrfCounter = 0;
  const nextCsrfToken = () => `csrf-story-11-5-fe-${++csrfCounter}`;
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (method === 'GET' && pathname === '/api/v1/accounts/14/positions/list') {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(successEnvelope(quotePositions)),
      });
      return;
    }
...
const recordDemo = async (videoTempDir) => {
  ...
  try {
    ...
    const centerChart = async () => {
      const chart = page.getByTestId('portfolio-dashboard-quote-ticker-chart');
      await chart.waitFor();
    ...
    await page.getByTestId('portfolio-quote-source-mode').waitFor();
    await wait(1_000);
    await centerChart();

    for (const symbol of ['000660', '035420', '005930']) {
      await page.getByTestId(`portfolio-symbol-${symbol}`).click();
      await page.getByTestId('portfolio-quote-source-mode').waitFor();
      await centerChart();
      await wait(1_100);
    }
...
const videoTempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'fixyz-fe-story-11-5-'));
